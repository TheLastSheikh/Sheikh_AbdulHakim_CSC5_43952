...
void bgnGame (int carDeck [], int carChnc[][2], int plyHand[], char cPlyHnd[], int delHand[], char cDlHand[], int ai1Hand[], char cai1Hnd[], int ai2Hand[], char cai2Hnd[]){
        shuffle(carDeck);
...
//Declare Variables
 ...
 //13 Cards in deck starting from Ace = 1 to King = 13
    ...
    bgnGame (carDeck, carChnc, plyHand, cPlyHnd, delHand, cDlHand, ai1Hand, cai1Hnd, ai2Hand, cai2Hnd);
 ...
 bool gameEnd = false;
  string endGame = "";
 while(plyTurn){
 dsplyUI(cPlyHnd, cDlHand, cai1Hnd, cai2Hnd, ai1Hand, ai2Hand, plyHand, delHand, plyCash, delCash, ai1Cash, ai2Cash, false);
 ...
 while(ai1Turn){
     if (sum(ai1Hand) <= 11){
         //ai1 hits
         ptCrdHn(carDeck, carChnc, cai1Hnd, ai1Hand);
     }else{
         ai1Turn = false;
     }
 }
 while(ai2Turn){
     if (sum(ai2Hand) <= 11){
         //ai2 hits
         ptCrdHn(carDeck, carChnc, cai2Hnd, ai2Hand);
     }else{
         ai2Turn = false;
     }
 }
 ...
 if (sum(delHand) == sum(plyHand) && !bust(ai2Hand)){
     ...
     dsplyUI(cPlyHnd, cDlHand, cai1Hnd, cai2Hnd, ai1Hand, ai2Hand, plyHand, delHand, plyCash, delCash, ai1Cash, ai2Cash, true);
     cout << endGame << endl;
     out << endGame << endl;
     cout << gameEnd2 << endl;
 gameEnd = true;
